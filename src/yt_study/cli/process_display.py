...
from yt_study.cli.formatters import print_cost_summary, print_run_summary
from yt_study.cli.process_context import CliProcessContext
from yt_study.cli.types import _BatchJobResult, _OrderedBatchFailure, _WorkerSlotManager
...
UI_STATUS_MAP: dict[EventType, _DashboardStatusFn] = {
    ...
    EventType.TRANSCRIPT_FETCHED: lambda title, _: (
        f"[green]✓ {title}... (Transcript Ready)[/green]"
    ),
    ...
    EventType.QUIZ_COMPLETE: lambda title, _: (
        f"[green]✓ {title}... (Quiz Ready)[/green]"
    ),
    EventType.GENERATION_COMPLETE: lambda title, _: (
        f"[green]✓ {title}... (Generated)[/green]"
    ),
}
