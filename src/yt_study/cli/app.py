...
def process(
    ...
    try:
        ...
        from yt_study.cli.types import (
            ResolvedSource,
            _BatchJobResult,
            _BatchVideoJob,
        ...
        from yt_study.services.pipeline import PipelineSharedState, dedupe_video_ids
        ...
        def _print_failure_panel(
            ...
            *,
            ...
            renderables.append(
                Text(f"Current log: {get_session_log_path()}", style="dim")
            )
        ...
        def _print_run_summary(result: PipelineResult, dashboard: Any) -> None:
            ...
            console.print(f"[dim]Current log: {get_session_log_path()}[/dim]\n")
        ...
        async def _prepare_source(single_url: str) -> ResolvedSource:
            ...
            except (PlaylistError, VideoUnavailableError) as e:
                raise UserVisibleCliError(
                    "Playlist Error",
                    [(parsed.playlist_id, str(e))],
                ) from e

            playlist_name, _ = await get_playlist_info(
                parsed.playlist_id,
                selected_cookie_file,
            )
        ...
        async def _run_single_url(single_url: str) -> bool:
            ...
            def on_event(event: PipelineEvent) -> None:
                ...
                slot = slot_manager.get(vid)

                if event.event_type == EventType.METADATA_START:
                    assigned = slot_manager.acquire(vid)
                    ...
                    released = slot_manager.release(vid)
                    ...
                    try:
                        ...
                        result = cast(
                            ...
                            await pipeline.run(
                                [fallback_video_id],
                                on_event=(
                                    on_batch_event if dashboard is not None else None
                                ),
                            ),
                        )
