...
def select_model(provider_key: str, available_models: Dict[str, List[str]]) -> str:
    ...
    while True:
        ...
        if total_pages > 1:
            console.print(
                "[dim]Type 'n' for next page, 'p' for previous page, or model number to select[/dim]"
            )
...
def get_api_key(provider_key: str, existing_key: Optional[str] = None) -> str:
    ...
    console.print(
        f"[dim]Get your API key from:[/dim] [link={provider['api_url']}]{provider['api_url']}[/link]\n"
    )
...
def run_setup_wizard(force: bool = False) -> Dict[str, str]:
    ...
    console.print(
        f"[green]✓ Found {sum(len(m) for m in available_models.values())} models across {len(available_models)} providers[/green]"
    )
