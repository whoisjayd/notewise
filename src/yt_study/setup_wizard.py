...
def select_provider(available_models: dict[str, list[str]]) -> str:
    ...
    for i, provider_key in enumerate(providers_list, 1):
        ...
        table.add_row(str(i), config["name"], f"{model_count} models")
    
    console.print(table)
    console.print(f"\n[dim]Total providers: {len(providers_list)}[/dim]")
    
    while True:
        choice = Prompt.ask("\nSelect provider", choices=[str(i) for i in range(1, len(providers_list) + 1)])
...
def select_model(provider_key: str, available_models: dict[str, list[str]]) -> str:
    ...
    while True:
        ...
        for i, model in enumerate(page_models, start_idx + 1):
            ...
            elif "pro" in model.lower() or "turbo" in model.lower():
        ...
        console.print(f"\n[dim]Page {current_page + 1}/{total_pages} | Showing {start_idx + 1}-{end_idx} of {len(models)} models[/dim]")
        
        if total_pages > 1:
            console.print("[dim]Type 'n' for next page, 'p' for previous page, or model number to select[/dim]")
        ...
        if choice.lower() == 'n' and current_page < total_pages - 1:
            ...
            if provider_key == "gemini" and not selected.startswith("gemini/") and not selected.startswith("vertex_ai/"):
                 return f"gemini/{selected}"
                 
            return selected


def get_api_key(provider_key: str, existing_key: Optional[str] = None) -> str:
    """Prompt for API key."""
    provider = PROVIDER_CONFIG[provider_key]
    
    console.print(f"\n[bold yellow]API Key Required:[/bold yellow] {provider['name']}")
    console.print(f"[dim]Get your API key from:[/dim] [link={provider['api_url']}]{provider['api_url']}[/link]\n")
    
    if existing_key:
        masked = f"{existing_key[:8]}...{existing_key[-4:]}" if len(existing_key) > 12 else "***"
...
def run_setup_wizard(force: bool = False) -> dict[str, str]:
    ...
    console.print(f"[green]✓ Found {sum(len(m) for m in available_models.values())} models across {len(available_models)} providers[/green]")
    ...
    # Get API key
    provider = PROVIDER_CONFIG[provider_key]
    existing_key = config.get(provider["env_var"])
    ...
    console.print(f"\n[dim]Selected model:[/dim] [cyan]{model}[/cyan]")
    console.print(f"[dim]You can now run:[/dim] [cyan]yt-study \"YOUTUBE_URL\"[/cyan]")
    console.print(f"[dim]To reconfigure:[/dim] [cyan]yt-study setup --force[/cyan]\n")
