...
from ..youtube.transcript import fetch_transcript, split_transcript_by_chapters, YouTubeIPBlockError
...
class PipelineOrchestrator:
    ...
    def validate_provider(self) -> bool:
        ...
        if key_name:
            ...
            if not os.environ.get(key_name):
                ...
                console.print("[dim]Please check your .env file or run:[/dim] [cyan]yt-study setup[/cyan]\n")
                return False
        ...
        async with self.semaphore:
            ...
            try:
                ...
                if use_chapters:
                    ...
                    for i, (chap_title, chap_text) in enumerate(chapter_transcripts.items(), 1):
                ...
                if progress and local_task_id is not None:
                    progress.update(local_task_id, description=f"[red]✗ {(video_title or video_id)[:20]}... {err_display}[/red]", visible=True)
    ...
    async def _process_with_dashboard(
        ...
        with Live(dashboard, refresh_per_second=10, console=console, screen=False):
            ...
            if not is_single_video:
                dashboard.update_overall_status(f"[cyan]📋 Fetching metadata for {len(video_ids)} videos...[/cyan]")
    ...
    def _print_summary(self, dashboard):
        ...
        summary_table = Table(title="📊 Processing Summary", border_style="cyan", show_header=True, header_style="bold magenta")
    ...
    async def run(self, url: str) -> None:
        ...
        try:
            ...
            if parsed.url_type == 'video':
                ...
                playlist_title, _ = await asyncio.to_thread(get_playlist_info, parsed.playlist_id)
