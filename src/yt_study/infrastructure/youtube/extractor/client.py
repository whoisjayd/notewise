...
class YouTubeExtractorClient:
    ...
    def _fetch_text(self, url: str) -> str:
        ...
        try:
            with self._opener.open(req, timeout=REQUEST_TIMEOUT_SECONDS) as resp:
                body = resp.read()
    ...
    def _extract_context(
        ...
        if not client.get("clientName"):
            client["clientName"] = INNERTUBE_CLIENT_NAME
        if not client.get("clientVersion"):
            client["clientVersion"] = INNERTUBE_CLIENT_VERSION
        client["hl"] = "en"
        client["timeZone"] = "UTC"
    ...
    def _first_key(node: Any, key: str) -> dict[str, Any] | None:
        found = YouTubeExtractorClient._find_key(node, key)
