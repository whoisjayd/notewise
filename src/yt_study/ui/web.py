...
class WebVisualizer:
    ...
    def refresh_data(self) -> None:
        ...
        for item in self.output_dir.iterdir():
            if item.is_dir():
                ...
                is_video_dir = (item / f"{item.name}.md").exists() or (
                    item / "chapters"
                ).exists()
                ...
                if is_video_dir:
                    ...
                    for sub in item.iterdir():
                        if sub.is_dir() and (
                            (sub / f"{sub.name}.md").exists()
                            or (sub / "chapters").exists()
                        ):
                            videos.append(sub)
                    if videos:
    ...
    def _find_node(
        self, nodes: list[dict[str, Any]], target_id: str
    ) -> dict[str, Any] | None:
        for n in nodes:
            if n["id"] == target_id:
                return n
    ...
    def load_content(self, path: Any, video_id: str | None) -> None:
        if path is None:
            return
        path = Path(path)
    ...
    def render(self) -> None:
                ...
                document.addEventListener('click', function(e) {
                    ...
                    if (target) {
                        ...
                        if (player) {
                            ...
                            const params = "?enablejsapi=1&autoplay=1&start=";
                            player.src = baseUrl + params + seconds;
                        }
                    }
                });
            </script>
        ...
        with ui.header().classes(
            "bg-slate-900 border-b border-slate-800 items-center justify-between"
        ):
            ...
            with ui.row().classes("items-center gap-2"):
                ...
                ui.button(
                    ...
                    on_click=lambda: ui.navigate.to(
                        "https://github.com/jayss/yt-study", new_tab=True
                    ),
                ).props("flat color=slate-400")

        with ui.splitter(value=20, limits=(15, 40)).classes(
            ...
            with (
                ...
                self.tree = ui.tree(
                    self.tree_data, label_key="label", on_select=self.handle_tree_select
            ...
            with (
                main_splitter.after,
                ui.splitter(value=60, limits=(30, 80)).classes(
                    "w-full h-full"
                ) as content_splitter,
            ):
                ...
                with (
                    ...
                    self.markdown_view = ui.markdown().classes(
                        "prose max-w-none w-full"
                    )

                    with ui.column().classes(
                        "w-full gap-4 hidden"
                    ) as self.editor_container:
                        self.editor = (
                            ui.textarea(label="Markdown Content")
                            .classes("w-full h-[60vh] font-mono")
                            .props('outlined dark autogrow color="blue-5"')
                        )
                        with ui.row().classes("gap-2"):
                            self.save_btn = ui.button(
                                "Save", icon="save", on_click=self.save_content
                            ).props("color=blue-600")
                            ui.button(
                                "Cancel",
                                icon="close",
                                on_click=lambda: self.load_content(
                                    self.current_path, self.current_video_id
                                ),
                            ).props("flat color=slate-400")

                # Video Area (Right side of right pane)
                with (
                    content_splitter.after,
                    ui.column().classes(
                        "w-full h-full bg-slate-900 p-6 "
                        "border-l border-slate-800 overflow-y-auto"
                    ),
                    ui.column().classes("w-full sticky top-0 gap-6"),
                ):
                    ui.label("VIDEO PLAYER").classes(
                        "text-xs font-bold text-slate-500 uppercase tracking-widest"
                    )
                    with ui.card().classes(
                        "w-full aspect-video bg-black p-0 overflow-hidden "
                        "shadow-2xl rounded-xl border border-slate-700"
                    ):
                        self.video_player = ui.html(
                            '<div class="flex items-center justify-center '
                            'w-full h-full text-slate-600 italic">'
                    ...
                    with ui.column().classes("gap-2"):
                        ...
                        with ui.row().classes("gap-2"):
                            ui.button(
                                ...
                                icon="open_in_new",
                                on_click=lambda: ui.navigate.to(
                                    f"https://youtube.com/watch?v={self.current_video_id}",
                                    new_tab=True,
                                )
