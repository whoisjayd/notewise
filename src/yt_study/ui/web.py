...
class WebVisualizer:
    ...
    def render(self):
        with ui.header().classes('items-center justify-between'):
            ...
            with ui.row():
                ui.button('Refresh', on_click=self.refresh).props('flat color=white icon=refresh')
        ...
        with ui.left_drawer(value=True).classes('bg-slate-100') as drawer:
            ...
            for p in self.projects:
                if p["playlist"] != current_playlist:
                    ...
                    ui.label(current_playlist or "Single Videos").classes('text-sm font-bold mt-4 mb-1 text-slate-500 uppercase')

                ui.button(p["title"], on_click=lambda p=p: self.select_project(p)).props('flat align=left').classes('w-full')

        with ui.column().classes('w-full p-4') as self.content_area:
            if not self.projects:
                ui.label('No projects found in output directory.').classes('text-xl italic text-slate-400')
            else:
                ui.label('Select a project from the sidebar to view details.').classes('text-xl italic text-slate-400')
        ...
        with self.content_area:
            ...
            with ui.row().classes('w-full gap-4 items-start'):
                ...
                with ui.column().classes('flex-1 max-w-3xl'):
                    ...
                    ui.label(f"Video ID: {project['id']}").classes('text-sm text-slate-500')

                # Content Section
                with ui.column().classes('flex-1 min-w-[400px] max-h-[80vh] overflow-y-auto'):
                    ui.label('Study Notes').classes('text-2xl font-bold mb-4 border-b w-full')
                    ...
                    def replace_ts(match):
                        ...
                        return f'<a href="javascript:void(0)" onclick="document.getElementById(\'yt-player\').src=\'{self._get_video_url(project["id"], seconds)}&autoplay=1\'" class="text-blue-600 hover:underline font-mono">[{ts}]</a>'

                    html_content = ui.markdown(content).classes('prose max-w-none')

                    # NiceGUI's markdown doesn't easily support custom link handling for JS calls inside the markdown block
                    # without some hacks or post-processing.
                    # Alternative: use a regex to find timestamps and wrap them in something we can intercept.
                    # For now, let's keep it simple. If we want clickable, we might need to process the markdown ourselves.

                    # Improved timestamp handling:
                    # We can use ui.html if we want full control, or just use markdown and inject some JS to handle clicks.
                    ui.run_javascript("""
                        document.addEventListener('click', function(e) {
                            if (e.target && e.target.tagName === 'A' && e.target.textContent.match(/\[\d{1,2}:\d{2}(:\d{2})?\]/)) {
                                const ts = e.target.textContent.replace('[', '').replace(']', '');
                                ...
                                player.src = baseUrl + '?enablejsapi=1&start=' + seconds + '&autoplay=1';
                                e.preventDefault();
                            }
                        });
                    """)
...
def start_web_ui(port: int = 8080, host: str = "0.0.0.0", output_dir: Optional[Path] = None):
