...
export function Nav({ version }: { version: string }) {
  ...
  useEffect(() => {
    ...
    // A section counts as "current" once it crosses a fixed line below the
    // sticky header, rather than whichever has the most pixels on screen --
    // that reads as the wrong section active on tall sections near the top.
          ...
          <nav aria-label="Primary" className="hidden md:flex items-center gap-7">
            {items.map((it) => (
              <a
                ...
                className={cn(
                  "hover-underline text-[13px] transition-colors duration-150",
                  active === it.href
                    ? "text-stamp [background-size:100%_1px]"
