...
title: CI/CD
sidebarTitle: CI/CD
description: GitHub Actions workflows, quality gates, and release process.
icon: git-branch
---
...
<AccordionGroup>
  <Accordion title="ci-main.yml — Full validation" icon="shield-check" defaultOpen>
    ...
    | `type-check` | `ty check` |
    | `startup-smoke` | `yt-study version` — ensures the package imports cleanly |
  ...
  <Accordion title="pr-gate.yml — PR validation" icon="git-pull-request">
    Runs on every pull request to `main`. Calls `ci-main.yml` as a reusable workflow. All jobs must pass before merge.
  </Accordion>

  <Accordion title="ghcr.yml — Docker image" icon="box">
    Builds and pushes the Docker image to `ghcr.io/whoisjayd/yt-study` on every push to `main` and on version tags.
  </Accordion>

  <Accordion title="release.yml — PyPI release" icon="upload">
  ...
  <Accordion title="Supporting workflows" icon="settings">
...
<Steps>
  ...
  <Step title="Startup smoke">
    `yt-study version` — package imports without error.
  </Step>
...
<Steps>
  <Step title="Bump the version">
    Update `__version__` in `src/yt_study/__init__.py` and `version` in `pyproject.toml`.
  </Step>
