...
sidebarTitle: Setup
description: Set up a local development environment for yt-study.
icon: laptop
---
...
<Tabs>
  <Tab title="Formatting">
    ```bash
    uv run ruff format --check src/yt_study tests   # check
    uv run ruff format src/yt_study tests            # auto-fix
    ```
  </Tab>
  <Tab title="Linting">
    ```bash
    uv run ruff check src/yt_study tests             # check
    uv run ruff check --fix src/yt_study tests       # auto-fix
    ```
  </Tab>
...
uv run yt-study process "URL"
```

Or create `~/.yt-study/config.env` via `uv run yt-study setup`.
...
make build
# Produces:
# dist/yt_study-X.Y.Z-py3-none-any.whl
# dist/yt_study-X.Y.Z.tar.gz
```
