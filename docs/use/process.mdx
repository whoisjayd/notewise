...
By default, `notewise process` shows a Rich live dashboard while work is running. The dashboard header shows the source, selected output folder, and the current session log path on its own full-width row so the path doesn't truncate. Below that, the dashboard summarizes safe runtime flags, progress counts, active video workers, chapter workers, and recent completions or failures. Secret values are never shown: API keys are summarized as present or missing, and cookie files are displayed by safe status/basename only.
