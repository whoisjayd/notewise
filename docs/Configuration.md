...
| `TEMPERATURE`           | LLM response temperature (0.0 to 1.0).           | `0.7`                     |
| `MAX_TOKENS`            | Maximum tokens for LLM responses.                | `None` (Model default)    |

### Advanced Tuning

These settings control how transcripts are processed before being sent to the LLM.

| Variable | Description | Default |
| :--- | :--- | :--- |
| `CHUNK_SIZE` | Target size (in tokens) for transcript chunks. | `4000` |
| `CHUNK_OVERLAP` | Overlap (in tokens) between chunks to preserve context. | `200` |
