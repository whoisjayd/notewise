...
keywords: ["notewise process", "youtube video", "process options", "study notes"]
---
...
| `--language`, `-l` | `en` | Request a transcript language. Can be repeated. |
...
<Note>
  Chapter-aware generation is automatic when YouTube chapters exist. By default, the final notes are bundled into one output file. Use `--chapter-directory-output` only when you want a folder of per-chapter Markdown files.
</Note>
