---
title: "Install NoteWise"
sidebarTitle: "Install"
description: "Install the NoteWise CLI with uv, pipx, pip, Docker, or the short binary installer."
keywords: ["install notewise", "notewise.click/install", "uv", "pipx", "pip", "docker", "python"]
---

NoteWise requires Python `>=3.10` when installed from PyPI. The CLI entry point is `notewise`.

This page only covers installation choices. Pick one method, verify the CLI is available, then continue to [Quickstart](/start/quickstart) for provider setup and your first video.

<Tabs sync={false}>
  <Tab title="uv tool">
    ```bash
    uv tool install notewise
    notewise version
    ```
  </Tab>
  <Tab title="uvx">
    ```bash
    uvx notewise --help
    ```
  </Tab>
  <Tab title="pipx">
    ```bash
    pipx install notewise
    notewise version
    ```
  </Tab>
  <Tab title="pip">
    ```bash
    python -m pip install notewise
    python -m notewise version
    ```
  </Tab>
  <Tab title="Docker">
    ```bash
    docker run --rm ghcr.io/whoisjayd/notewise:latest version
    ```
  </Tab>
</Tabs>

## Standalone installers

Use the standalone installer when you prefer a release binary instead of a Python tool environment. Browser visits show install options; `curl`/`wget` receive the POSIX installer; PowerShell receives the Windows installer. The release installers download the latest GitHub release asset and verify checksums.

<CodeGroup>
```bash macOS or Linux
curl -fsSL https://notewise.click/install | sh
```

```powershell Windows PowerShell
irm https://notewise.click/install | iex
```

</CodeGroup>

Set `NOTEWISE_INSTALL_DIR` before running an installer to choose the install location.

## Verify

```bash
notewise version
```

If `notewise version` prints a version, installation is done. Continue with [Quickstart](/start/quickstart) to create config and process one public video.
