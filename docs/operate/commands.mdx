...
`add` and `delete` are fully interactive: any argument or option left out is
prompted for instead of erroring. `update` is interactive when `<name>` is
omitted; when `<name>` is passed on the command line, it still requires
`--base-url`, `--api-key`, or both.
