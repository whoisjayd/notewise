...
Omit `VIDEO_ID` from `cache show` and NoteWise lists recently cached videos with a number and prompts for a pick.
...
The active session log path is also shown directly in the [live dashboard](/use/process#live-dashboard) header while `notewise process` is running.
