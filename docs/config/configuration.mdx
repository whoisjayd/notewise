...
other saved endpoints remain unchanged. Names are unique after normalization. Base
URLs must be absolute `http://` or `https://` URLs; setup accepts a URL with or
without `/v1` and stores it with the `/v1` suffix.
