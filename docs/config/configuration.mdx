...
time; the `KEY` argument is optional on all three — omit it and NoteWise
lists every allowed key with a number and prompts for a pick. `set` validates
the new value immediately and reports an error without leaving the process in
a broken state if it's invalid (e.g. `TEMPERATURE` out of range). `notewise
config edit` is an interactive, category-grouped
...
Adding or updating an endpoint here immediately syncs `DEFAULT_MODEL` to
match, without an extra confirmation, whenever `DEFAULT_MODEL` is unset or
already points at that endpoint. The same paginated, searchable model picker
backs model selection in `notewise setup`, this `DEFAULT_MODEL` category, and
`notewise inference add|update` — type a number, search text, `n`/`p` to page,
or press the Left/Right arrow keys to jump pages instantly without pressing
Enter. `edit-config`
