...
These docs track NoteWise `1.7.0`. Source defaults are Python `>=3.11`, model `gemini/gemini-2.5-flash`, output directory `./output`, output format `md`, temperature `0.7`, max concurrent videos `5`, YouTube request limit `10/min`, chunk size `4000`, overlap `200`, and max concurrent chapters `3`.
