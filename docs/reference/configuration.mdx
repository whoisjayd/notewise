...
description: Complete reference for all yt-study configuration settings, defaults, and CLI overrides.
icon: settings
---

<Info>
  This is the canonical reference for all config keys and defaults. For the getting-started walkthrough, see [Configuration](/getting-started/configuration).
</Info>
...
| 2 | `~/.yt-study/config.env` | Written by `yt-study setup` |
...
See [LLM Providers](/concepts/providers) for the full routing table.
...
OUTPUT_DIR=/data/yt-study-output
```
...
These constants are not exposed in `config.env`. Change them by editing `src/yt_study/_constants.py`.
