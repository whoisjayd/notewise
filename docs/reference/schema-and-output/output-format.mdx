...
The main notes artifact now supports `md`, `html`, `pdf`, and `docx` via `--format`.
You can request one format or a comma-separated set such as `--format md,html,pdf`.
...
<Tabs>
  ...
  <Tab title="Chapter-Aware Video">
    ...
    <Note>
      `--format md` keeps the existing per-chapter folder layout. `--format html`, `pdf`, and `docx` bundle the generated chapter notes into a single rendered document in the output root. Mixed requests like `md,html` generate both layouts.
    </Note>
  </Tab>
...
### Main notes file (`<title>.<format>`)

The study notes are generated in clean Markdown first, then written directly as `.md` or rendered to `.html`, `.pdf`, or `.docx` with a shared document theme for tighter spacing and more readable typography:
