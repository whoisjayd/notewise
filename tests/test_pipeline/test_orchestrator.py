...
class TestPipelineOrchestrator:
    ...
    def orchestrator(self, temp_output_dir, mock_llm_provider):
        with patch("yt_study.pipeline.orchestrator.get_provider", return_value=mock_llm_provider):
            ...
            orch.generator.generate_chapter_based_notes = AsyncMock(return_value="# Chapter Notes")
            orch.generator.provider = mock_llm_provider # needed for direct calls in chapter loop
            return orch
    ...
    def test_validate_provider_missing_key(self, orchestrator, monkeypatch):
        ...
        with patch("yt_study.config.config.get_api_key_name_for_model", return_value="TEST_KEY"):
    ...
    async def test_process_video_single(self, orchestrator):
        ...
        # Mock dependencies
        with patch("yt_study.pipeline.orchestrator.get_video_title", return_value="Test Video"), \
             patch("yt_study.pipeline.orchestrator.get_video_duration", return_value=100), \
             patch("yt_study.pipeline.orchestrator.get_video_chapters", return_value=[]), \
             patch("yt_study.pipeline.orchestrator.fetch_transcript", new_callable=AsyncMock) as mock_fetch:
    ...
    async def test_process_video_with_chapters(self, orchestrator):
        ...
        with patch("yt_study.pipeline.orchestrator.get_video_title", return_value="Long Video"), \
             patch("yt_study.pipeline.orchestrator.get_video_duration", return_value=4000), \
             patch("yt_study.pipeline.orchestrator.get_video_chapters", return_value=["chap1"]), \
             patch("yt_study.pipeline.orchestrator.fetch_transcript", new_callable=AsyncMock) as mock_fetch, \
             patch("yt_study.pipeline.orchestrator.split_transcript_by_chapters", return_value={"Ch1": "text"}):
            
            mock_transcript = MagicMock()
            mock_fetch.return_value = mock_transcript
            
            video_id = "vid123"
            output_path = orchestrator.output_dir / "ignored.md" # Folder structure used instead
    ...
    async def test_run_video_flow(self, orchestrator):
        ...
        with patch("yt_study.pipeline.orchestrator.parse_youtube_url") as mock_parse, \
             patch.object(orchestrator, "_process_with_dashboard", new_callable=AsyncMock) as mock_dash:
