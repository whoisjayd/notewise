...
class TestConfigIO:
    ...
    def test_load_config_corrupted(self):
        ...
        with patch("pathlib.Path.exists", return_value=True), \
             patch("builtins.open", mock_open(read_data="junk data without equals sign")):
    ...
    def test_save_config(self):
        ...
        with patch("yt_study.setup_wizard.load_config", return_value={"OLD_KEY": "old_val"}), \
...
class TestInteractiveFlow:
    ...
    def test_select_provider(self):
        ...
        with patch("yt_study.setup_wizard.PROVIDER_CONFIG", test_config), \
    ...
    def test_select_model_pagination(self):
        ...
        with patch("yt_study.setup_wizard.PROVIDER_CONFIG", {"p1": {"name": "P1"}}), \
    ...
    def test_select_model_gemini_prefix(self):
        ...
        with patch("yt_study.setup_wizard.PROVIDER_CONFIG", {"gemini": {"name": "Google"}}), \
...
class TestWizardOrchestration:
    ...
    def test_run_setup_wizard_full_flow(self):
        ...
        # Mocks
        with patch("yt_study.setup_wizard.load_config", return_value={}), \
             patch("yt_study.setup_wizard.get_available_models", return_value={"gemini": ["gemini-pro"]}), \
             patch("yt_study.setup_wizard.select_provider", return_value="gemini"), \
             patch("yt_study.setup_wizard.select_model", return_value="gemini/gemini-pro"), \
    ...
    def test_run_setup_wizard_skip_existing(self):
        ...
        with patch("yt_study.setup_wizard.load_config", return_value={"exists": "true"}), \
             patch("rich.prompt.Confirm.ask", return_value=False): # Do not reconfigure
