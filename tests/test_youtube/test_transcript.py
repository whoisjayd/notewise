...
class TestFetchTranscript:
    ...
    async def test_fetch_transcript_fallback_auto(self, mock_transcript_api_instance):
        ...
        # Manual raises error
        mock_list.find_manually_created_transcript.side_effect = NoTranscriptFound("id", [], [])
    ...
    @pytest.mark.asyncio
    async def test_fetch_transcript_fallback_translation(self, mock_transcript_api_instance):
        ...
        # All finds fail
        mock_list.find_manually_created_transcript.side_effect = NoTranscriptFound("id", [], [])
        mock_list.find_generated_transcript.side_effect = NoTranscriptFound("id", [], [])
        ...
        mock_translated.fetch.return_value = [{'text': 'Hello', 'start': 0.0, 'duration': 1.0}]
