...
class TestPlaylistExtraction:
    ...
    async def test_extract_playlist_retry_success(self, mock_pytube):
        ...
        mock_fail = MagicMock()
        type(mock_fail).video_urls = PropertyMock(side_effect=Exception("Network Error"))
        
        mock_success = MagicMock()
        type(mock_success).video_urls = PropertyMock(return_value=["https://youtube.com/watch?v=vid1"])
